...
def process_one_shard(corpus_params, params):
    ...
    i, (src_shard, seg_shard, speaker_shard, relation_shard, tgt_shard, maybe_id, filter_pred) = params
    # create one counter per shard
    sub_sub_counter = defaultdict(Counter)
    assert len(src_shard) == len(seg_shard) == len(speaker_shard) == len(relation_shard) == len(tgt_shard)
    logger.info("Building shard %d." % i)
    dataset = inputters.Dataset(
        fields,
        readers=([src_reader, src_reader, src_reader, src_reader, tgt_reader]
                 if tgt_reader else [src_reader]),
        data=([("src", src_shard), ("seg", seg_shard), ("speaker", speaker_shard), ("relation", relation_shard),
               ("tgt", tgt_shard)]
              if tgt_reader else [("src", src_shard)]),
        dirs=([opt.src_dir, None, None, None, None]
...
def build_save_dataset(corpus_type, fields, src_reader, tgt_reader, opt):
    ...
    if corpus_type == 'train':
        ...
        tgts = opt.train_tgt
        ids = opt.train_ids
        segs = opt.train_seg  # path
        speakers = opt.train_speaker  # path
        relations = opt.train_relation  # path
        ...
        ids = [None]
        segs = [opt.valid_seg]
        speakers = [opt.valid_speaker]
        relations = [opt.valid_relation]
    ...
    def shard_iterator(srcs, segs, speakers, relations, tgts, ids, existing_shards,
                       existing_fields, corpus_type, opt):
        """
        Builds a single iterator yielding every shard of every corpus.
            srcs ['data/temp/temp_input.txt']
            tgts ['data/temp/temp_output.txt']
                segs []
                speakers []
                relations []
            ids [None]
            ...
            existing_shards []
        """
        for src, seg, speaker, relation, tgt, maybe_id in zip(srcs, segs, speakers, relations, tgts, ids):
            # useless
            ...
            seg_shards = split_corpus(seg, opt.shard_size)
            speaker_shards = split_corpus(speaker, opt.shard_size)
            relation_shards = split_corpus(relation, opt.shard_size)
            tgt_shards = split_corpus(tgt, opt.shard_size)

            for i, (src_s, seg_s, speaker_s, relation_s, tgt_s) in enumerate(
                    zip(src_shards, seg_shards, speaker_shards, relation_shards, tgt_shards)):
                yield (i, (src_s, seg_s, speaker_s, relation_s, tgt_s, maybe_id, filter_pred))

    shard_iter = shard_iterator(srcs, segs, speakers, relations, tgts, ids, existing_shards,
                                existing_fields, corpus_type, opt)
