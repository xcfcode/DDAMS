...
class CopyGenerator(nn.Module):
    ...
    * :math:`p_{copy}` the probility of copying a particular word.
    ...
    def __init__(self, input_size, pre_out_size, output_size, pad_idx):
        super(CopyGenerator, self).__init__()

        self.pre_linear = nn.Linear(input_size, pre_out_size)
        self.linear = nn.Linear(pre_out_size, output_size)
    ...
    def forward(self, hidden, attn, src_map):
        ...
        logits = self.pre_linear(hidden)  # hidden-->pre_out_size
        logits = self.linear(logits)  # pre_out_size-->vocab_size
