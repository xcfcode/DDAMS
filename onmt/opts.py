...
def model_opts(parser):
    ...
    group.add('--word_vec_size', '-word_vec_size', type=int, default=300,
    ...
    group.add('--encoder_type', '-encoder_type', type=str, default='hier3',
              choices=["hier3"],
              help="Type of encoder layer to use.")

    group.add('--speaker_type', '-speaker_type', type=str, default='ami',
              choices=["ami", "icsi"],
              help="used for speaker num ami 5 icsi 11")

    group.add('--gnn_type', '-gnn_type', type=str, default='gat',
              choices=['ggnn', 'gat'],
              help="Type of graph encoder. Options are "
                   "[ggnn|gat].")

    group.add('--gnn_layers', '-gnn_layers', type=int, default=1,
    ...
    group.add('--enc_layers', '-enc_layers', type=int, default=1,
              help='Number of layers in the encoder')
    group.add('--dec_layers', '-dec_layers', type=int, default=1,
              help='Number of layers in the decoder')
    group.add('--rnn_size', '-rnn_size', type=int, default=200,
              help="Size of rnn hidden states. Overwrites "
                   "enc_rnn_size and dec_rnn_size")
    group.add('--enc_rnn_size', '-enc_rnn_size', type=int, default=200,
              help="Size of encoder rnn hidden states. "
                   "Must be equal to dec_rnn_size except for "
                   "speech-to-text.")
    group.add('--dec_rnn_size', '-dec_rnn_size', type=int, default=200,
    ...
    group.add('--lambda_coverage', '-lambda_coverage', type=float, default=0,
    ...
    group.add('--apex_opt_level', '-apex_opt_level', type=str, default="O0",
              choices=["O0", "O1", "O2", "O3"],
...
def preprocess_opts(parser):
    ...
    group.add('--train_seg', '-train_segs', required=True, nargs='+',
              help="Path(s) to the training segs data")
    group.add('--train_speaker', '-train_speakers', required=True, nargs='+',
              help="Path(s) to the training speakers data")
    group.add('--train_relation', '-train_relations', required=True, nargs='+',
              help="Path(s) to the training relations data")

    group.add('--valid_seg', '-valid_segs',
              help="Path(s) to the valid segs data")
    group.add('--valid_speaker', '-valid_speakers',
              help="Path(s) to the valid speakers data")
    group.add('--valid_relation', '-valid_relations',
              help="Path(s) to the valid relations data")
    ...
    group.add('--shard_size', '-shard_size', type=int, default=20,
    ...
    group.add('--src_vocab_size', '-src_vocab_size', type=int, default=10000,
              help="Size of the source vocabulary")
    group.add('--tgt_vocab_size', '-tgt_vocab_size', type=int, default=10000,
    ...
    group.add('--src_words_min_frequency',
              '-src_words_min_frequency', type=int, default=2)
    group.add('--tgt_words_min_frequency',
              '-tgt_words_min_frequency', type=int, default=2)
    ...
    group.add('--src_seq_length', '-src_seq_length', type=int, default=15000,
              help="Maximum source sequence length")
    group.add('--src_seq_length_trunc', '-src_seq_length_trunc',
              type=int, default=10000,
              help="Truncate source sequence length.")
    group.add('--tgt_seq_length', '-tgt_seq_length', type=int, default=700,
...
def train_opts(parser):
    ...
    group.add('--seed', '-seed', type=int, default=777,
    ...
    group.add('--pre_word_vecs_dec', '-pre_word_vecs_dec',
              help="If a valid path is specified, then this will load "
                   ...
                   "See README for specific formatting instructions.")
    # group.add('--pre_word_vecs_enc', '-pre_word_vecs_enc', default='data/ami_embeddings.enc.pt',
    #           help="If a valid path is specified, then this will load "
    #                "pretrained word embeddings on the encoder side. "
    #                "See README for specific formatting instructions.")
    # group.add('--pre_word_vecs_dec', '-pre_word_vecs_dec', default='data/ami_embeddings.dec.pt',
    #           help="If a valid path is specified, then this will load "
    #                "pretrained word embeddings on the decoder side. "
    #                "See README for specific formatting instructions.")
    # Fixed word vectors
    ...
    group.add('--valid_steps', '-valid_steps', type=int, default=100,
    ...
    group.add('--max_grad_norm', '-max_grad_norm', type=float, default=2,
              help="If the norm of the gradient vector exceeds this, "
                   "renormalize it to have the norm equal to "
                   "max_grad_norm")
    group.add('--dropout', '-dropout', type=float, default=[0.2], nargs='+',
    ...
    group.add('--start_decay_steps', '-start_decay_steps',
              ...
              help="Start decaying every decay_steps after "
                   "start_decay_steps")
    group.add('--decay_steps', '-decay_steps', type=int, default=800,
              help="Decay every decay_steps")

    group.add('--decay_method', '-decay_method', type=str, default="none",
              choices=['noam', 'noamwd', 'rsqrt', 'none'],
              help="Use a custom decay rate.")
    group.add('--warmup_steps', '-warmup_steps', type=int, default=800,
...
def translate_opts(parser):
    ...
    group.add('--seg', '-seg', required=True,
              help="")
    group.add('--speaker', '-speaker', required=True,
              help="")
    group.add('--relation', '-relation', required=True,
              help="")
    ...
    group.add('--beam_size', '-beam_size', type=int, default=5,
              help='Beam size')
    group.add('--min_length', '-min_length', type=int, default=300,
              help='Minimum prediction length')
    group.add('--max_length', '-max_length', type=int, default=450,
