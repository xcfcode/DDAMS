...
class Translator(object):
    ...
    def __init__(
        ...
        self.start_t = self._tgt_vocab.stoi["<t>"]
        self.end_t = self._tgt_vocab.stoi["</t>"]

        self._tgt_vocab_len = len(self._tgt_vocab)
        self._gpu = gpu
        self._use_cuda = gpu > -1
        self._dev = torch.device("cuda", self._gpu) \
            if self._use_cuda else torch.device("cpu")  # torch.device
    ...
    def _gold_score(self, batch, memory_bank, src_lengths, memory_bank_utterance, memory_lengths_utterance,
                    hier_matrix, src_vocabs,
                    use_src_map, enc_states, batch_size, src):
        if "tgt" in batch.__dict__:
            gs = self._score_target(
                batch, memory_bank, src_lengths, memory_bank_utterance, memory_lengths_utterance,
                hier_matrix, src_vocabs,
    ...
    def translate(
        ...
        data = inputters.Dataset(
            ...
            readers=([self.src_reader, self.src_reader, self.src_reader, self.src_reader, self.tgt_reader]
                     if tgt else [self.src_reader, self.src_reader, self.src_reader, self.src_reader]),
            data=[("src", src), ("seg", seg), ("speaker", speaker), ("relation", relation),
                  ("tgt", tgt)] if tgt else [("src", src), ("seg", seg), ("speaker", speaker), ("relation", relation)],
    ...
    def _translate_random_sampling(
        ...
        # Encoder forward.
        src, enc_states, memory_bank, src_lengths, memory_bank_utterance, lengths_utterance, hier_matrix = self._run_encoder(
            batch)
        self.model.decoder.init_state(src, memory_bank, enc_states)
        ...
        results = {
            ...
            "gold_score": self._gold_score(
                batch, memory_bank, src_lengths, memory_bank_utterance, lengths_utterance,
                hier_matrix, src_vocabs, use_src_map,
        ...
        for step in range(max_length):
            ...
            log_probs, attn = self._decode_and_generate(
                ...
                memory_bank_utterance=memory_bank_utterance,
                memory_lengths_utterance=lengths_utterance,
                hier_matrix=hier_matrix,
                src_map=src_map,
                step=step,
    ...
    def _run_encoder(self, batch):
        ...
        enc_states, memory_bank, src_lengths, memory_bank_utterance, lengths_utterance, hier_matrix = self.model.encoder(
            src, batch.seg, batch.speaker, batch.adj_coo, batch.edge_type, batch.rels, src_lengths)
        ...
        return src, enc_states, memory_bank, src_lengths, memory_bank_utterance, lengths_utterance, hier_matrix
    ...
    def _decode_and_generate(
            ...
            src_vocabs,
            memory_lengths,
            memory_bank_utterance=None,
            memory_lengths_utterance=None,
            hier_matrix=None,
            src_map=None,
            step=None,
            batch_offset=None):
        ...
        dec_out, dec_attn = self.model.decoder(
            ...
            memory_lengths=memory_lengths,
            memory_bank_utterance=memory_bank_utterance,
            memory_lengths_utterance=memory_lengths_utterance,
            hier_matrix=hier_matrix,
            step=step
        )
    ...
    def _translate_batch(
        ...
        if isinstance(memory_bank, tuple):
            ...
            mb_device = memory_bank.device
            memory_bank_utterance = tile(memory_bank_utterance, beam_size, dim=1)


        memory_lengths = tile(src_lengths, beam_size)

        memory_utterance_lengths = tile(lengths_utterance, beam_size)
        hier_matrix_beam = tile(hier_matrix, beam_size)
        ...
        for step in range(max_length):
            ...
            log_probs, attn = self._decode_and_generate(
                ...
                memory_lengths=memory_lengths,
                memory_bank_utterance=memory_bank_utterance,
                memory_lengths_utterance=memory_utterance_lengths,
                hier_matrix=hier_matrix_beam,
                src_map=src_map,
                step=step,
    ...
    def _score_target(self, batch, memory_bank, src_lengths, memory_bank_utterance, memory_lengths_utterance,
                      hier_matrix, src_vocabs, src_map):
        tgt = batch.tgt
        tgt_in = tgt[:-1]
    ...
    def _report_score(self, name, score_total, words_total):
        if words_total == 0:
            ...
            try:
                ans = math.exp(-score_total / words_total)
