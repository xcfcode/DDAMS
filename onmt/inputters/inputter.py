...
def make_src(data, vocab):
    # when in field.process.numericalize
    src_size = max([t.size(0) for t in data])  # for example in data
...
def get_fields(
    ...
    src_field_kwargs = {"n_feats": n_src_feats,
                        ...
                        "base_name": "src"}
    fields["src"] = fields_getters[src_data_type](**src_field_kwargs)  # TextMultiField
    ...
    fields["seg"] = RawField()
    fields["speaker"] = RawField()
    fields["relation"] = RawField()
    fields["adj_coo"] = RawField()
    fields["rels"] = RawField()
    fields["edge_type"] = RawField()
    ...
    if dynamic_dict:
        ...
        fields["src_map"] = src_map  # Field

        src_ex_vocab = RawField()
        fields["src_ex_vocab"] = src_ex_vocab  # RawField

        align = Field(
            use_vocab=False, dtype=torch.long,
            postprocessing=make_tgt, sequential=False)  # Field
