...
def process_file(id, filename_prefix):
    ...
    if not annotations.has_key("relation"):
        relations = []
    else:
        relations = annotations["relation"]
    schema = annotations["schema"] if annotations.has_key("schema") else []
    ...
    for item in schema:
        ...
        cdu = []
        if item["positioning"].has_key("embedded-unit"):
            ...
            for edu in cdu:
                if not edus.has_key(edu):
                    cdu.remove(edu)
        if item["positioning"].has_key("embedded-schema"):
...
def process_dialogue(dialogue):
    ...
    for relation in dialogue["relations"]:
        def get_head(x):
            if dialogue["edus"].has_key(x): return x
            else: 
                for du in dialogue["cdus"][x]:
                    if not has_incoming.has_key(du): return get_head(du)
